...
def get_comm(g: nx.Graph, q: int, k: int) -> set[int]:
    def update_lowers(subg: nx.Graph, lowers):
    ...
    def update_uppers(subg: nx.Graph, uppers):
        def local_core(v):
            nums = [0] * (uppers[v] + 1)
    ...
    def update_subgraph(g: nx.Graph, subg: nx.Graph, lowers, uppers, q, k):
    ...
    update = True
    while update and uppers[q] >= k:
        # print(f"{lowers[q]=}, {uppers[q]=} {subg.number_of_nodes()=}")

        update_lower = update_lowers(subg, lowers)
        update_upper = update_uppers(subg, uppers)

        subg, update_subg = update_subgraph(g, subg, lowers, uppers, q, k)
